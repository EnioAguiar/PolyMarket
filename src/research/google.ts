...
export class GoogleAdapter implements ResearchSource {
  id = 'google';
  name = 'Google Search + News';
  category = SourceCategory.NEWS;
  rating = 3 as const;  // Default rating for news
  ...
  private getTimeRestrict(marketTimeHorizon?: number): string | null {
    ...
    if (hours <= 1) return 'h1';  // Last hour
    if (hours <= 6) return 'h6';  // Last 6 hours
    if (hours <= 24) return 'd1';  // Last day
    return null;  // No restriction for longer
  }
  ...
  private calculateConfidence(signal: { totalResults: number }, marketTimeHorizon?: number): number {
    ...
    if (marketTimeHorizon && marketTimeHorizon <= 6 * 60 * 60 * 1000) {
      confidence += 0.1;  // Recent news more valuable for short-term
    }
  ...
  private calculateRecency(marketTimeHorizon?: number): number {
    ...
    if (hours <= 1) return 1.0;  // Very fresh needed
