...
6. 🟡 **Guard de geoblock no startup — parcialmente feito** (21/set/2026): `src/api/geoblock.ts` checa `GET https://polymarket.com/api/geoblock` via `https.get` (não `fetch` — só assim herda o patch de proxy) antes de `createClobClient()` em `src/index.ts`; se `blocked: true`, a variável `geoblocked` desliga o trading real pelo mesmo caminho `else` que já existia pro `dryRun` — `createClobClient()` fica inalcançável, confirmado por inspeção estática. Testado ao vivo: retornou `{"blocked":false,"country":"IN","region":"MH"}`, confirmando que o guard enxerga o egress real via proxy. **O que falta**: o pedido original era "abortar/**alertar via Telegram**" — o que foi implementado é só `logger.error()` (log estruturado, sem notificação nenhuma) e o processo **não aborta**, só roda em modo degradado (mesmo caminho do `dryRun`, bankroll fica 0). Se isso acontecer em produção sem ninguém olhando o log, ninguém fica sabendo. Falta: chamada ao Telegram quando `geoblocked === true`. Implementado via `docs/superpowers/plans/2026-09-21-infra-resilience.md` (commit `6f95f531`) — plano não pediu Telegram/abort nesse texto, é uma lacuna herdada do item original.
