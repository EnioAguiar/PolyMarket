...
## Sub-projeto 4: Sistema de Sinal de Pesquisa (Research Signal System) — validado, resultado honesto: ainda não bate o acaso
...
**Resultado real do backtest (Task 8, 4 rodadas até chegar numa medição válida — as 3 primeiras tinham um bug real no lançamento do interpretador Python que fazia todo artigo cair pra julgamento só-por-manchete; corrigido e re-rodado; depois um follow-up de quebra por tipo de mercado achou e corrigiu um segundo bug real no classificador — ver abaixo):**

Backtest da estratégia `sentiment` contra 100 mercados resolvidos reais da Gamma API, filtrados para pares Yes/No genuínos (43 de 100; o resto eram Over/Under, handicaps e pares por nome de time, que não mapeiam pra uma pergunta YES/NO fixa):

- **Taxa de acerto real: 12/34 corretos (35,3%)** nos mercados onde a Jev teve notícia real pra julgar (8 dos 43 mercados Yes/No não tinham nenhum artigo antes do `beforeDate` e foram excluídos do número por padrão default 0,5/YES, contados à parte).
- **35,3% está abaixo dos 50% de acerto esperado só por acaso.** Este é o achado central e não é positivo — a abordagem específica de agregação de sentimento testada aqui não bate o acaso nesta amostra.
- **Quebra por tipo de mercado (follow-up pós-fechamento, achado real e não trivial):** um revisor apontou que a amostra parecia dominada por uma escada de mercados de limiar de preço BTC/ETH correlacionados, o que misturaria duas tarefas bem diferentes num só número. Ao tentar medir isso, achamos e corrigimos **um segundo bug real**: `extractCryptoThreshold()` (classificador do Task 1) exigia um `$` literal antes do número, mas a formulação real da Polymarket pra esses mercados de curtíssimo prazo omite o cifrão (`"Ethereum above 2,670 on September 22, 3PM ET?"`, sem `$`) — então o classificador falhava silenciosamente e nenhum desses mercados era reconhecido como cripto (corrigido em `src/research/classify.ts` pra aceitar número puro com separador de milhar, ex. `2,670`, sem exigir `$`; isso também destrava o `resolution_sniping` real pra essa formulação, que antes nunca disparava nela). Com o classificador corrigido:
  - **Mercados de limiar cripto (BTC/ETH, ex. `Bitcoin above 88,200 on September 22, 3PM ET?`): 12/28 corretos (42,9%)** — abaixo do acaso, mas mais perto que o número agregado sugeria.
  - **Outros mercados de limiar (ações/commodities, ex. `Will Coinbase (COIN) hit (HIGH) $200 in September?`, `Will Silver hit (HIGH) $66 in September?`): 0/6 corretos (0,0%)** — amostra pequena demais pra tirar conclusão estatística, mas nenhum acerto.
  - **Achado honesto adicional, mais importante que o split em si**: nenhum dos 34 mercados com sinal real era uma pergunta de evento genuíno (eleição, aprovação regulatória, evento geopolítico) — os 34 eram 100% perguntas de limiar de preço, só que em dois grupos de ativo (cripto vs ações/commodities). O rótulo "outros mercados" **não significa "eventos de notícia genuínos"** como a hipótese original supunha — significa "limiar de preço de um ativo não-cripto". A conclusão "sentimento não bate o acaso" está validada especificamente pra perguntas de limiar de preço de curto prazo; ainda não temos uma amostra real de mercados de evento genuíno pra testar essa estratégia contra eles, porque o feed de mercados resolvidos da Gamma API, nesta janela de tempo, simplesmente não trouxe nenhum.
- **Taxa de texto completo: 43 de 105 artigos julgados (41,0%) usaram texto real crawleado do artigo; 59,0% caíram pro fallback de manchete-só**, por bloqueio real anti-bot de vários sites de notícia (ex.: Reuters retorna captcha DataDome real, confirmado ao vivo, não é um bug de ferramenta).
- **Limitação real de amostragem**: os 43 mercados Yes/No sobreviventes eram majoritariamente uma escada correlacionada de mercados de limiar de preço BTC/ETH no mesmo horário (ex. `Ethereum above 2,670/2,680/.../2,830`, `Bitcoin above 84,400/84,600/.../88,400`) — múltiplos strikes do mesmo ativo no mesmo timestamp, não uma amostra i.i.d. de perguntas independentes. Isso é uma limitação real de como a Gamma API devolve "os 100 mercados resolvidos mais recentes por id", não um bug de código.

**Leitura honesta:** o pipeline mecânico funciona de ponta a ponta — chamadas reais à Jev, busca real de notícias com guarda de vazamento de data validada por amostragem, dados reais de preço, metodologia real de backtest. Mas a abordagem de sentimento testada, nesta amostra específica (100% perguntas de limiar de preço, cripto e não-cripto), **não demonstrou sinal preditivo melhor que cara-ou-coroa**. Não é uma vitória disfarçada de sucesso — é uma resposta honesta e um tanto desanimadora à pergunta que o sub-projeto existia pra responder, com o adendo de que a pergunta original ("sentimento presta pra prever mercados da Polymarket?") só foi testada, até agora, contra o subtipo "limiar de preço de curto prazo" — não contra eventos de notícia genuínos, por falta de amostra real disponível no momento do teste.
...
10. 🔶 **Decisão de provider de IA tomada (22/set/2026): TypeSafe/Jev, não MiniMax.** `src/ai/minimax.ts` (a conta MiniMax cotada aqui antes não tem plano/crédito ativo, segue não usada) foi substituída como decisão de provider pelo sub-projeto 4: `src/ai/jev.ts`, com `TYPESAFE_API_KEY` real no `.env`, confirmado funcionando em chamadas ao vivo. **O pipeline de research (3 estratégias, Google News RSS, Crawl4AI, backtest) foi construído e validado com dados reais** — ver seção "Sub-projeto 4" acima. Mas a validação em si deu um resultado honesto e não positivo: a estratégia de sentimento testada acertou 12/34 (35,3%) em mercados resolvidos reais, **abaixo do acaso (50%)** — quebrando por tipo, 42,9% em limiares de preço cripto (n=28) e 0% nos outros limiares de preço não-cripto (n=6, amostra pequena). Por isso **nada disso foi conectado ao caminho de decisão de trading** — isso continua sendo uma decisão futura distinta (sub-projeto 5, ainda em aberto), e só faria sentido revisitar depois de melhorar o sinal (amostra de mercados mais diversa/independente incluindo eventos genuínos, taxa de texto completo maior) o suficiente pra bater o acaso de forma consistente. O bot em produção continua decidindo só por preço/liquidez.
