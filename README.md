...
**Resultado real do backtest (Task 8, 4 rodadas até chegar numa medição válida — as 3 primeiras tinham um bug real no lançamento do interpretador Python que fazia todo artigo cair pra julgamento só-por-manchete; corrigido e re-rodado):**

Backtest da estratégia `sentiment` contra 100 mercados resolvidos reais da Gamma API, filtrados para pares Yes/No genuínos (47 de 100; o resto eram Over/Under, handicaps e pares por nome de time, que não mapeiam pra uma pergunta YES/NO fixa):

- **Taxa de acerto real: 14/39 corretos (35,9%)** nos mercados onde a Jev teve notícia real pra julgar (8 dos 47 mercados Yes/No não tinham nenhum artigo antes do `beforeDate` e foram excluídos do número por padrão default 0,5/YES, contados à parte).
- **35,9% está abaixo dos 50% de acerto esperado só por acaso.** Este é o achado central e não é positivo — a abordagem específica de agregação de sentimento testada aqui não bate o acaso nesta amostra.
- **Taxa de texto completo: 26 de 115 artigos julgados (22,6%) usaram texto real crawleado do artigo; 77,4% caíram pro fallback de manchete-só**, por bloqueio real anti-bot de vários sites de notícia (ex.: Reuters retorna captcha DataDome real, confirmado ao vivo, não é um bug de ferramenta).
- **Limitação real de amostragem**: os 47 mercados Yes/No sobreviventes eram majoritariamente uma escada correlacionada de mercados de limiar de preço BTC/ETH no mesmo horário (ex. `Ethereum above 2,650/2,660/.../2,810`, `Bitcoin above 83,800/84,000/.../87,800`) — múltiplos strikes do mesmo ativo no mesmo timestamp, não uma amostra i.i.d. de perguntas independentes. Isso é uma limitação real de como a Gamma API devolve "os 100 mercados resolvidos mais recentes por id", não um bug de código.

**Leitura honesta:** o pipeline mecânico funciona de ponta a ponta — chamadas reais à Jev, busca real de notícias com guarda de vazamento de data validada por amostragem, dados reais de preço, metodologia real de backtest. Mas a abordagem de sentimento testada, nesta amostra específica, **não demonstrou sinal preditivo melhor que cara-ou-coroa**. Não é uma vitória disfarçada de sucesso — é uma resposta honesta e um tanto desanimadora à pergunta que o sub-projeto existia pra responder.
