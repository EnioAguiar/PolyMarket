...
9. ~~Terminar o checkpoint de teste real (antigo `01-03`)~~ ✅ **feito**: aposta real colocada e resolvida (ver item 1), `txHash` disponível via `transactionsHashes` da resposta do CLOB (campo array, não string única — corrigido também no código, ver `OrderExecutionResult`).
