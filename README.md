...
## Sub-projeto 4: Sistema de Sinal de Pesquisa (Research Signal System) — validado, resultado honesto: sem edge em limiar de preço, inconclusivo pra evento genuíno (amostra não achada)
...
**Resultado real do backtest (Task 8, 4 rodadas até chegar numa medição válida; depois 2 rodadas de follow-up pós-fechamento pra investigar a composição da amostra — 3 rodadas ao todo, cada uma com sua própria amostra da Gamma API, números reportados separados, não misturados):**

**Rodada 1 (fechamento do Task 8)** — 100 mercados resolvidos, 39 pares Yes/No genuínos: **14/39 corretos (35,9%)**, abaixo do acaso. 26/115 artigos (22,6%) com texto completo.
...
**Rodada 3 (follow-up, amostra ampliada pra caçar mercado de evento genuíno)** — um revisor apontou, corretamente, que as rodadas 1 e 2 pareciam ser 100% perguntas de limiar de preço (cripto e ações/commodities), sem nenhum evento genuíno (eleição, aprovação, evento geopolítico) — o que tornaria qualquer conclusão sobre "sentimento funciona pra eventos" não testada de verdade. Ampliamos a busca pra 500 mercados resolvidos (5 páginas via paginação nova em `fetchMarkets`), e **caçamos especificamente por mercados que não fossem limiar de preço de nenhum tipo** (não só cripto). A rodada rodou por 20 minutos e não terminou (processou 176 dos ~215 pares Yes/No esperados antes do timeout do script de diagnóstico) — os números abaixo são reais, de dados já pagos e coletados, só que de uma amostra parcial, não os 500 completos:
  - **Limiar de preço cripto (BTC/ETH), n=123: 49/123 corretos (39,8%)**, cobrindo só **5 timestamps de vencimento distintos** — cada ladder de 10-15 strikes no mesmo horário é uma correlação forte do mesmo caminho de preço, não 123 tentativas independentes. Efetivamente mais perto de 5 eventos reais testados do que 123.
  ...
  - **Mercados de evento genuíno encontrados mesmo após ampliar 5x: zero.** Um candidato apareceu (`"Will US Dollar Index (DXY) hit (LOW) 100.60 Week of September..."`) mas na checagem manual também é limiar de preço — só que com um número decimal sem vírgula (`100.60`, valor abaixo de 1.000), que escapou do detector genérico do script (`PRICE_THRESHOLD_PATTERN`, que exigia `$` ou separador de milhar). Corrigido o detector pra aceitar também número decimal solto; reclassificado pro bucket de "outros limiares" acima.
  - **Combinado da rodada 3: 51/139 corretos (36,7%)**, abaixo do acaso — mas as barras de erro em cada bucket individual são grandes o bastante (principalmente n=16 e os efetivos ~5 eventos cripto) que **nenhum dos dois buckets é estatisticamente distinguível de 50% de acaso puro** com esse tamanho de amostra. Não dá pra afirmar "sentimento funciona melhor em cripto que em ações" com esses números — os dois são ruído dentro da margem esperada.

**Leitura honesta e final, juntando as 3 rodadas:** o pipeline mecânico funciona de ponta a ponta em dados reais — busca de notícia real, julgamento real via Jev, preço real, backtest real. Mas **as 3 rodadas, juntas, nunca conseguiram testar a estratégia de sentimento contra um mercado de evento genuíno de verdade** — mesmo ampliando a busca 5x, o estoque de mercados resolvidos Yes/No que a Gamma API está retornando agora é inteiramente dominado por perguntas de limiar de preço de curto prazo (cripto de hora em hora, ações/commodities de mês), que são estruturalmente uma tarefa diferente (e mais dura) do que "vai acontecer o evento X". Isso significa que a pergunta original do sub-projeto — "sentimento de notícia presta pra prever mercado da Polymarket?" — **continua sem resposta definitiva pro tipo de mercado que a estratégia foi pensada pra atacar**. O que está validado, com razoável confiança (3 rodadas, ~250 avaliações reais no total): **contra perguntas de limiar de preço de curtíssimo prazo, sentimento de notícia não bate consistentemente o acaso** — o que nem é surpreendente, já que essas perguntas resolvem por movimento técnico de preço, não por informação nova. Não é uma vitória disfarçada de fracasso nem um fracasso disfarçado de vitória — é uma resposta parcial e honesta, com uma lacuna real e nomeada: falta achar (ou esperar aparecer no feed) um lote de mercados de evento genuíno resolvidos pra testar a pergunta que o sub-projeto realmente queria responder.
...
10. 🔶 **Decisão de provider de IA tomada (22/set/2026): TypeSafe/Jev, não MiniMax.** `src/ai/minimax.ts` (a conta MiniMax cotada aqui antes não tem plano/crédito ativo, segue não usada) foi substituída como decisão de provider pelo sub-projeto 4: `src/ai/jev.ts`, com `TYPESAFE_API_KEY` real no `.env`, confirmado funcionando em chamadas ao vivo. **O pipeline de research (3 estratégias, Google News RSS, Crawl4AI, backtest) foi construído e validado com dados reais** — ver seção "Sub-projeto 4" acima. Mas a validação em si (3 rodadas, ~250 avaliações reais) deu um resultado parcial e honesto, não uma vitória: contra perguntas de limiar de preço de curto prazo (cripto e ações/commodities), sentimento de notícia não bate o acaso de forma consistente (35-40% de acerto nas rodadas, dentro da margem de ruído estatístico pra essas amostras pequenas). **Mais importante: nenhuma das 3 rodadas, mesmo ampliando a busca 5x, achou um mercado de evento genuíno resolvido pra testar** — o estoque atual de mercados resolvidos Yes/No da Gamma API é dominado por perguntas técnicas de preço, não eventos tipo eleição/aprovação. A pergunta original ("sentimento presta pra prever evento genuíno?") segue sem resposta real por falta de amostra, não por resultado negativo. Por isso **nada disso foi conectado ao caminho de decisão de trading** — isso continua sendo uma decisão futura distinta (sub-projeto 5, ainda em aberto), e só faria sentido revisitar depois de achar/esperar uma amostra real de eventos genuínos resolvidos pra testar a pergunta de verdade. O bot em produção continua decidindo só por preço/liquidez.
