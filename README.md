...
9. ✅ **Checkpoint de teste real (antigo `01-03`) — fechado, testado ao vivo de ponta a ponta, incluindo assinatura de ordem real**: primeira rodada (sem proxy) rodei `placeMarketOrder()` do próprio bot contra mercado líquido de verdade ($5.6M vol/24h), $1 de teste. Achei e corrigi um bug real no caminho: `placeMarketOrder()`/`placeLimitOrder()` retornavam `success: true` incondicionalmente sem checar `result.success`/`errorMsg` do CLOB (`src/api/clob.ts`, commit `0e6f70fc`). Com o fix, a resposta real apareceu: 403 "Trading restricted in your region" — **mas geoblock bloqueia antes de qualquer verificação de assinatura, então isso sozinho não provava nada sobre `POLY_1271`**. Testei de novo com o proxy da Índia aplicado (`proxy-agent`, ver sub-projeto 2) — **e a ordem foi aceita de verdade**: `{"success":true,"orderID":"0xcdab1c1d...","status":"delayed"}`. **Isso sim prova a assinatura `POLY_1271` funcionando numa ordem real.** No caminho, achei e corrigi outro bug (documentado no spec do sub-projeto 2): setar `HTTP_PROXY`/`HTTPS_PROXY` como env var literal faz o axios (usado pelo CLOB client) tentar aplicar proxy HTTP puro numa URL SOCKS5, quebrando a autenticação — corrigido lendo de uma variável `PROXY_URL` separada, passada explícita via `getProxyForUrl`. **Achado extra**: `polymarket.com/api/geoblock` disse `blocked:false, country:CH` no IP sem proxy, mas a submissão de ordem real no mesmo IP deu 403 — os dois endpoints discordam pra esse IP específico; o guard do sub-projeto 2 não confia cegamente no endpoint de geoblock público por causa disso. Notificação do Telegram ainda não testada.
