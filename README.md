...
9. 🔴 **Checkpoint de teste real (antigo `01-03`) — testado ao vivo, resultado: bloqueado por geoblock, não por bug de assinatura**: rodei `placeMarketOrder()` do próprio bot (não mais a UI) contra um mercado líquido de verdade ($5.3M volume/24h), $1 de teste, `POLY_1271` configurado. **Achado no caminho**: o código tinha um bug real — `placeMarketOrder()`/`placeLimitOrder()` retornavam `success: true` incondicionalmente sempre que a chamada não lançava exceção, **sem checar `result.success`/`result.errorMsg` da resposta real do CLOB**. Corrigido (`src/api/clob.ts`, commit `0e6f70fc`) — agora com o fix, a resposta real apareceu: `{"error":"Trading restricted in your region... https://docs.polymarket.com/developers/CLOB/geoblock","status":403}`. Saldo não mudou ($3.28 antes e depois — nada foi gasto, ordem rejeitada de verdade). **Conclusão: a assinatura `POLY_1271` funciona (a ordem foi aceita, autenticada e processada até a checagem de geoblock — não foi rejeitada por auth) — quem bloqueou foi o geoblock da própria Polymarket na hora de submeter ordem nova**, batendo com a doc oficial (regiões restritas não abrem ordem nova, só fecham existente). Isso é evidência direta de que o sub-projeto 2 (proxy + geoblock guard) não é só teórico — é o que está travando o bot agora mesmo. Falta ainda: confirmar a notificação do Telegram (não testado nesta rodada).
